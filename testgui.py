...
faceClassif = cv2.CascadeClassifier(cv2.data.haarcascades + "haarcascade_russian_plate_number.xml")
...
cap = None
root = Tk()
lblInfo1 = Label(root, text="SNC IIOT-B14 Steaming Paining Counter", font="bold")
lblInfo1.grid(column=0, row=0, columnspan=2)
selected = IntVar()
rad1 = Radiobutton(root, text="Manual",width=20, value=1, variable=selected, command=video_de_entrada)
rad2 = Radiobutton(root, text="Auto",width=20, value=2, variable=selected, command=video_de_entrada)
...
btnEnd = Button(root, text="Exit Programs", state="disabled", command=finalizar_limpiar)
