...
H36TL2B = False
H36TL1B = False
commandstart = input("Enter Model: ")
if commandstart == 'H36TL2B':
    def H36TL2_B():
        global H36TL2B
        H36TL2B = True
        return H36TL2B
    H36TL2_B()
elif commandstart == 'H36TL1B':
    def H36TL1_B():
        global H36TL1B
        H36TL1B = True
        return H36TL1B
...
def gen_frames():
    ...
    # NOTE : counter is used to count the number of frames
    # NOTE : Run the program counter system for model "H36TL1B"
    if H36TL1B == True and H36TL2B == False:
        while(1):  # For Model H36TL2 Blue
            ...
            cv2.putText(img, str("H36TL1B"), (20, 50),
            ...
            #** --- end of Model H36TL2 Blue --- **#
    # NOTE : Run the program counter system for model "H36TL1B"
    elif H36TL1B == False and H36TL2B == True:
        while(1):  # For Model H36TL1 Blue
            ret, img = cap.read()
            blur = cv2.blur(img, (25, 25))
            hsv = cv2.cvtColor(blur, cv2.COLOR_BGR2HSV)
            ...
            cv2.putText(img, str(counter), (550, 450),
                        cv2.FONT_HERSHEY_SIMPLEX, 2, text_color, 3, cv2.LINE_AA)
            cv2.putText(img, str("H36TL2B"), (20, 50),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2, cv2.LINE_AA)
            cv2.imshow("IIOT-B14 Camera Tracking Counter", img)
            if cv2.waitKey(10) & 0xFF == ord('q'):
                cap.release()
...
if __name__ == '__main__':
    app.run(host='172.20.10.7', port=9999)
